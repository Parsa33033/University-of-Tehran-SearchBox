 ...
 *
 */
public class Query implements IndexerInterface{
	
	private StopWords stopWords;
	private PorterStemmer stemmer;
	private static String query;
	private Map<String, Double> searchResult;
	 ...
	 */
	public Map<String, Double> run(String s) {
		this.query = "";
		...
		this.query = this.filterString(s);
...
//		for(int i = 0 ; i<str.length ; i++) {
//			if(this.invertedIndexCopy.containsKey(str[i])) {
//				this.invertedIndexCopy.get(str[i]).keySet().removeAll(intersect);
//				for(String s : this.invertedIndexCopy.get(str[i]).keySet()) {
//					if(!this.searchResult.containsKey(s)) {
//						this.searchResult.put(s, (double)0);
//					}
//					if(this.invertedIndexCopy.get(str[i]).containsKey(s))
//						searchResult.put(s,((double) this.searchResult.get(s)+ this.invertedIndexCopy.get(str[i]).get(s)) * Math.pow(Math.E, -15));
//				}
//			}
//		}
